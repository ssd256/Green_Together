...
<body>
    ...
    <section>
		...
		<div id="orderDiv">
			...
			<input type="hidden" class="cNo" name="c_no" value="${ c.cNo }">
			<table>
				<tr>
					<td class="title">구매상품정보</td>
					<td class="content"><input type="text" class="orderText" value="${ c.pName }, ${ c.cCount }개" disabled="disabled">
										<input type="hidden" name="orderCount" value="${ c.cCount }">
					</td>	
				</tr>
				<tr>
					<td class="title">총 상품가격</td>
					<td class="content">
							<input type="text" class="orderText" name="price" value="<fmt:formatNumber value="${ c.cPrice }" pattern="###,###,###"/>" disabled="disabled"> 원
							<input type="hidden" class="price" value="${ c.cPrice }">
					</td>	
				</tr>
				<c:if test="${ s.last }">
				<tr id="totalPay">
					...
					<td class="content">
										<input type="text" class="orderText" value="<fmt:formatNumber value="${ cPrice }" pattern="###,###,###" /> (배송비 2,500원 포함)" disabled="disabled"> 원
										<input type="hidden" name="orderPay" value="${ cPrice }"> 
					</td>
				</tr>
				</c:if>
			</table>
    ...
    <script>
		...
		// 결제 API
         function Pay(){
			...
			var c = document.getElementsByClassName('cNo');
			var cItem = [];
            ...
            IMP.request_pay({
                ...
                name : '주문명:결제테스트',
                amount : $('input[name="orderPay"]').val(),
                ...
                if (rsp.success) {
                   ...
                   jQuery.ajax({
                      url: "cartOrderInsert.po", //cross-domain error가 발생하지 않도록 동일한 도메인으로 전송
                      type: 'POST',
                      dataType: 'json',
                      data: {
                         //기타 필요한 데이터가 있으면 추가 전달
                         'imp_uid' : rsp.imp_uid,
                         'cItem' : $('input[name="cItem"]').val(),
                         'recipient' : $('input[name="recipient"]').val(),
                         'orderAddr' : $('input[name="orderAddr"]').val(),
                         'reciPhone' : $('input[name="reciPhone"]').val(),
                         'orderMsg' : $('input[name="orderMsg"]').val()
                      }, 
